...
const AcademicsTalks = () => {
  ...
  useEffect(() => {
    const fetchAcademicEvents = async () => {
      try {
        ...
        const filtered = data.filter((event: EventType) => event.is_Academic);
  ...
  return (
    <div className="container mx-auto px-4 py-8 min-h-screen">
      <h1 className="text-3xl font-bold mb-14 text-center">Academic Talks</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
        {academicEvents ? (
          academicEvents.length > 0 ? (
            ...
            <p className="text-center col-span-full">No academic talks found.</p>
          )
        ) : (
